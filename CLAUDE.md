...
### Cover art in the detail pane

- **The cover is read on its own thread, not through the worker.** `art_thread`
  is the update probe's pattern: nothing on screen is blocked on a picture, and
  a read is about 50ms of ffmpeg that must not reach the render loop. Not the
  command channel, because `serve` is not reading it during a pipeline, so a
  request queued there would arrive when the run ended — which for the pane is
  never. One long-lived thread rather than one per request, so a held `j`
  cannot spawn fifty ffmpegs: the reads queue and the ones for rows the cursor
  has already left are answered into a cache nobody looks at.
- **One ffmpeg call does the whole of it.** `tag::cover_art` goes from the
  tagged file to raw RGB at exactly the size asked for: ffmpeg already knows
  how to find a picture in a tagged file, it is a required dependency, and
  nothing large is held in memory or written to disk. Through a file rather
  than a pipe, like `shrink`, because `run_bounded` drains nothing until the
  child exits. A few kilobytes of rawvideo would fit in a pipe buffer today,
  which is exactly how that trap gets re-set by a later change to the size.
- **Half-blocks, so a cover is ordinary spans in the buffer.** `▀` with the
  upper pixel as the foreground and the lower as the background doubles the
  vertical resolution and, far more importantly, means `recolour`, `NO_COLOR`
  and a popup's `Clear` all handle a picture without knowing it is one. The
  Kitty graphics protocol would look better and would bypass the buffer, so
  the overlay would have album art showing through it and ratatui's diffing
  would not know those cells were dirty.
- **`Art::rows` is pixel rows and `cell_rows()` is the conversion.** Two
  numbers that have to agree is how they come to disagree, which they did once
  between the thread asking for a picture and the pane drawing it. The pane
  asks for `rows * 2` pixels and draws `cell_rows()` of them.
- **A cover is drawn only at exactly the size the pane asked for, and today
  that is a tautology.** `ART_MAX` is 24 and the pane only exists above
  `PREVIEW_FROM`, where the inner width is never below 39, so every cover is
  24 by 12 and no path produces a mismatch. The check is kept because it is
  what makes `ART_MAX` or `PREVIEW_FROM` safe to move later, which is exactly
  when nobody would think to look. Its test says so rather than pretending to
  cover a live case.
- **`App.art_size` is set by the draw and read by the loop,** the same bargain
  `viewport` makes with `page`: only the layout knows how much room the pane
  has. `None` also means "do not ask", which is how `NO_COLOR` skips the read
  entirely rather than reading a picture it would then decline to draw. There
  are three ways to have no room and the third is the one that was missed: no
  track under the cursor, a pane too short for `ART_FLOOR`, and **no pane at
  all** below `PREVIEW_FROM`, where `draw_detail` never runs and so cannot
  report anything. Left standing, that last one had the loop read the previous
  wide frame's size as room and spend an ffmpeg per settle on covers with
  nowhere to be drawn, invisibly, for as long as the terminal stayed narrow.
  The narrow branch clears it.
- **`art_room` takes `plain` rather than asking.** The depth is a process-wide
  `OnceLock`, so a test of it is a test of how the machine running the suite is
  configured. Same rule as `depth_from`.
- **The picture gives way before the words, and goes above them.** `ART_FLOOR`
  is what the metadata needs; below that the pane carries no cover, because it
  exists to say which track this is. Above rather than below, or a cover under
  a variable number of metadata rows would jump every time the cursor moved to
  a track with a different number of them.
- **`None` is cached like any other answer.** A folder of untagged files would
  otherwise pay for the read every time the cursor rested on a row.
- **The cache is cleared by `Msg::Tracks` and by `c`.** A new list is a new
  folder's worth of covers, and one folder is the natural bound — keeping every
  folder opened in a session is how a cache nobody thought about grows without
  limit. `c` is the only thing that changes a cover while the list is up
  (`cap_cover` runs during a pass, when nothing is cached), and it drops the
  old one on dispatch rather than on the reply, or the stale picture sits
  beside the picker choosing its replacement.
- **Gated on `can_command`, like `e`.** During a run the tags and the art are
  both still being written, so a picture read then is a picture of a
  half-finished track, and the read would land on a file ffmpeg is already
  busy with.
